...
-- CREACIÓN DE PROCEDIMIENTO ALMACENADOS PARA EL DASHBOAR Y GRAFICO

CREATE PROCEDURE SP_D_CantidadVentas
AS BEGIN
SELECT TOP 1* FROM Ventas ORDER BY Id_Venta DESC
END

CREATE PROCEDURE SP_D_Articulos
AS BEGIN
SELECT SUM(Cantidad) FROM Articulos
END

CREATE PROCEDURE SP_D_Grafico
AS BEGIN
